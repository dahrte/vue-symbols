...
onMounted(async () => {
  const module = await import('@vue-symbols/icons/files')
...
<template>
  <main class="-mb-px flex flex-1 flex-col">
    <Container render="section" class="flex w-full flex-1">
      ...
      <NotFound v-else-if="!filteredIcons.length" />

      <Grid v-else>
        <Card v-for="icon in filteredIcons" :key="icon.name" :icon="icon.icon" :name="icon.name" :size type="files" />
      </Grid>
