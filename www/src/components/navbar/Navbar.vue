...
import Size from '@/components/navbar/Size.vue'
import { TOTAL_FILE_ICONS, TOTAL_FOLDER_ICONS } from '@/data/svgs'
...
const placeholder = computed(() => {
  return route.path === '/'
    ? `Search ${TOTAL_FILE_ICONS} file icons...`
    : `Search ${TOTAL_FOLDER_ICONS} folder icons...`
})
</script>
  ...
  <nav class="bg-surface-base/70 sticky top-0 z-10 supports-backdrop-filter:backdrop-blur-lg">
    <Container class="border-b">
      <div class="grid grid-cols-1 md:grid-cols-[1fr_auto_14rem]">
        <Search :placeholder class="min-w-0" />
