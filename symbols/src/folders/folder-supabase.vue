<template>
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M19.2621 20.0692C19.0259 20.3446 18.5463 20.1945 18.5405 19.8427L18.4577 14.6989H22.1922C22.8688 14.6989 23.2458 15.4233 22.8252 15.9123L19.2621 20.0692Z"
      fill="url(#paint0_linear_24689_562)"
    />
    <path
      d="M19.2621 20.0692C19.0259 20.3446 18.5463 20.1945 18.5405 19.8427L18.4577 14.6989H22.1922C22.8688 14.6989 23.2458 15.4233 22.8252 15.9123L19.2621 20.0692Z"
      fill="url(#paint1_linear_24689_562)"
    />
    <path
      d="M17.7433 11.794C17.9795 11.5186 18.4577 11.6691 18.4635 12.0209L18.4998 17.1644H14.8118C14.1352 17.1644 13.7582 16.4402 14.1788 15.9509L17.7433 11.794Z"
      fill="#3ECF8E"
    />
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M5 4C3.34315 4 2 5.34315 2 7V17C2 18.6569 3.34315 20 5 20H13V18H5C4.44772 18 4 17.5523 4 17V7C4 6.44772 4.44772 6 5 6H7.78388C8.01539 6 8.23973 6.08033 8.41862 6.22727L10.8119 8.19318C11.3486 8.63402 12.0216 8.875 12.7161 8.875H19C19.5523 8.875 20 9.32272 20 9.875V10H22V9.875C22 8.21815 20.6569 6.875 19 6.875H12.7161C12.4846 6.875 12.2603 6.79467 12.0814 6.64773L9.6881 4.68182C9.15142 4.24098 8.47841 4 7.78388 4H5Z"
      fill="#64748B"
    />
    <defs>
      <linearGradient
        id="paint0_linear_24689_562"
        x1="18.4579"
        y1="15.8396"
        x2="21.6965"
        y2="17.3064"
        gradientUnits="userSpaceOnUse"
      >
        <stop stop-color="#249361" />
        <stop offset="1" stop-color="#3ECF8E" />
      </linearGradient>
      <linearGradient
        id="paint1_linear_24689_562"
        x1="16.9863"
        y1="13.9738"
        x2="18.3263"
        y2="16.698"
        gradientUnits="userSpaceOnUse"
      >
        <stop stop-opacity="0.2" />
        <stop offset="1" stop-opacity="0" />
      </linearGradient>
    </defs>
  </svg>
</template>
