<template>
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
      d="M5 4C3.34315 4 2 5.34315 2 7V17C2 18.6569 3.34315 20 5 20H13V18H5C4.44772 18 4 17.5523 4 17V7C4 6.44772 4.44772 6 5 6H7.78388C8.01539 6 8.23973 6.08033 8.41862 6.22727L10.8119 8.19318C11.3486 8.63402 12.0216 8.875 12.7161 8.875H19C19.5523 8.875 20 9.32272 20 9.875V10H22V9.875C22 8.21815 20.6569 6.875 19 6.875H12.7161C12.4846 6.875 12.2603 6.79467 12.0814 6.64773L9.6881 4.68182C9.15142 4.24098 8.47841 4 7.78388 4H5Z"
      fill="#64748B"
    />
    <path
      d="M16 19.52C16 20.86 17.09 21.95 18.43 21.95C19.77 21.95 20.86 20.86 20.86 19.52V16"
      stroke="#8B5CF6"
      stroke-width="1.5"
      stroke-linecap="round"
    />
    <path d="M16 19.52V16.89L17.72 18.61" stroke="#8B5CF6" stroke-width="1.5" stroke-linecap="round" />
    <path
      d="M20.86 15.55C21.7713 15.55 22.51 14.8113 22.51 13.9C22.51 12.9887 21.7713 12.25 20.86 12.25C19.9487 12.25 19.21 12.9887 19.21 13.9C19.21 14.8113 19.9487 15.55 20.86 15.55Z"
      stroke="#8B5CF6"
      stroke-width="1.5"
      stroke-linecap="round"
    />
  </svg>
</template>
